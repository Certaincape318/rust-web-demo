...
const UPDATE_SQL: &'static str = "update task set name=$1,content=$2,update_time=$3,status=$4 \
                                  where id=$5";
const CREATE_SQL: &'static str = "insert into task(create_time,name,content,update_time,status) \
                                  values($1,$2,$3,$4,$5)";
...
pub fn save(task: &Task) {
    let params: &[&ToSql] =
        &[&task.create_time, &task.name, &task.content, &task.update_time, &task.status, &task.id];
    if task.id > 0 {
